view: cc_sr {
  derived_table: {
    sql: WITH base_data AS (
          SELECT
              DATE(ti.created_on) AS created_date,
              ti.type,
              ti.status,
              CAST(
                  REPLACE(
                      JSON_QUERY(
                          ti.extended_info,
                          'strict $.MANDATE_EXECUTION_NUMBER'
                      ),
                      '"',
                      ''
                  ) AS INTEGER
              ) AS execution_no
          FROM
              team_product.looker_RM_CC ti

      ),
      aggregated_data AS (
      SELECT
      created_date,
      -- CREATE
      CASE
      WHEN COUNT(CASE WHEN type = 'CREATE' THEN 1 END) > 0
      THEN ROUND(
      COUNT(CASE WHEN type = 'CREATE' AND status = 'SUCCESS' THEN 1 END) * 100.0 /
      COUNT(CASE WHEN type = 'CREATE' THEN 1 END), 2
      )
      END AS create_sr,
      -- COLLECT - First Execution
      CASE
      WHEN COUNT(CASE WHEN type = 'COLLECT' AND execution_no = 1 THEN 1 END) > 0
      THEN ROUND(
      COUNT(CASE WHEN type = 'COLLECT' AND execution_no = 1 AND status = 'SUCCESS' THEN 1 END) * 100.0 /
      COUNT(CASE WHEN type = 'COLLECT' AND execution_no = 1 THEN 1 END), 2
      )
      END AS First_Exec_SR,
      -- COLLECT - Recurring Execution
      CASE
      WHEN COUNT(CASE WHEN type = 'COLLECT' AND execution_no > 1 THEN 1 END) > 0
      THEN ROUND(
      COUNT(CASE WHEN type = 'COLLECT' AND execution_no > 1 AND status = 'SUCCESS' THEN 1 END) * 100.0 /
      COUNT(CASE WHEN type = 'COLLECT' AND execution_no > 1 THEN 1 END), 2
      )
      END AS Recurring_Exec_SR,
      -- REVOKE
      CASE
      WHEN COUNT(CASE WHEN type = 'REVOKE' THEN 1 END) > 0
      THEN ROUND(
      COUNT(CASE WHEN type = 'REVOKE' AND status = 'SUCCESS' THEN 1 END) * 100.0 /
      COUNT(CASE WHEN type = 'REVOKE' THEN 1 END), 2
      )
      END AS revoke_sr
      FROM base_data
      GROUP BY created_date
      )
      SELECT
      created_date,
      CONCAT(CAST(create_sr AS VARCHAR), '%') AS "CREATE-SR",
      CONCAT(CAST(First_Exec_SR AS VARCHAR), '%') AS "1stEXEC-SR",
      CONCAT(CAST(Recurring_Exec_SR AS VARCHAR), '%') AS "RecurringEXEC-SR",
      CONCAT(CAST(revoke_sr AS VARCHAR), '%') AS "REVOKE-SR",
      -- Dynamically compute the average considering only non-null values
      CONCAT(CAST(ROUND(
      (COALESCE(create_sr, 0) + COALESCE(First_Exec_SR, 0) + COALESCE(Recurring_Exec_SR, 0) + COALESCE(revoke_sr, 0)) /
      NULLIF(
      (CASE WHEN create_sr IS NOT NULL THEN 1 ELSE 0 END +
      CASE WHEN First_Exec_SR IS NOT NULL THEN 1 ELSE 0 END +
      CASE WHEN Recurring_Exec_SR IS NOT NULL THEN 1 ELSE 0 END +
      CASE WHEN revoke_sr IS NOT NULL THEN 1 ELSE 0 END), 0),
      2
      ) AS VARCHAR), '%') AS "Average-SR"
      FROM aggregated_data
      ORDER BY created_date DESC
      ;;
  }

  suggestions: no

  measure: count {
    type: count
    drill_fields: [detail*]
  }

  dimension: created_date {
    type: date
    sql: ${TABLE}.created_date ;;
  }

  dimension: createsr {
    type: string
    sql: ${TABLE}."CREATE-SR" ;;
  }

  dimension: 1st_execsr {
    type: string
    sql: ${TABLE}."1stEXEC-SR" ;;
  }

  dimension: recurring_execsr {
    type: string
    sql: ${TABLE}."RecurringEXEC-SR" ;;
  }

  dimension: revokesr {
    type: string
    sql: ${TABLE}."REVOKE-SR" ;;
  }

  dimension: averagesr {
    type: string
    sql: ${TABLE}."Average-SR" ;;
  }

  set: detail {
    fields: [
      created_date,
      createsr,
      1st_execsr,
      recurring_execsr,
      revokesr,
      averagesr
    ]
  }
}
