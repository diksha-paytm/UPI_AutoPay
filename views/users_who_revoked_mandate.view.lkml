view: users_who_revoked_mandate {
  derived_table: {
    sql: WITH active_mandates AS (select *
      ...
      SELECT
      COALESCE(SUM(active_mandates.users_who_revoked_mandate), 0) AS "active_mandates.users_who_revoked_mandate"
  ...
  dimension: active_mandates_users_who_revoked_mandate {
    type: number
    sql: ${TABLE}."active_mandates.users_who_revoked_mandate" ;;
  }

  set: detail {
    fields: [active_mandates_users_who_revoked_mandate]
  }
}
