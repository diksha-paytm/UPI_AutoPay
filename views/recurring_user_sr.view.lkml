view: recurring_user_sr {
  derived_table: {
    sql: WITH base_data AS (
      ...
      handle,
      CASE
      WHEN COUNT(DISTINCT txn_key) > 0 THEN
      ROUND(
      ...
      SELECT
      created_date,
      MAX(CASE WHEN handle = 'paytm' THEN sr END) AS paytm_sr,
      ...
      SELECT
      created_date,
      CONCAT(CAST(paytm_sr AS VARCHAR), '%') AS "paytm SR",
      ...
      COALESCE(ptyes_sr, 0)) /
      NULLIF(
      (CASE WHEN paytm_sr IS NOT NULL THEN 1 ELSE 0 END +
